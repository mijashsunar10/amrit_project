        ...
        <form action="{{ route('ourteam.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    <label for="bio" class="block text-pastry-primary font-medium mb-2">Bio*</label>
                    <textarea id="bio" name="bio" rows="4" 
                              class="w-full px-4 py-2 border rounded-lg focus:ring-orange-500 focus:border-orange-500 @error('bio') border-red-500 @enderror">{{ old('bio') }}</textarea>
                    @error('bio')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
